...
class HapticForceManagerCB(Node):
    def __init__(self):
        ...
        # CLUTCH "Guided blending" manager (F=0, B=1): reference blending ON, but
        # NO assistive guidance FORCES on the handle. It is a copy of
        # haptic_force_manager_CFB with the goal-directed guidance channel
        # (F_guide velocity field + F_fixture funnel) DELETED from the applied
        # wrench; everything else (F_sync tether, clutch handling, grasp-follow,
        # joint-limit vibration, global damping) is left untouched. Because
        # ASSIST_BLENDING=True, main_shared_autonomy is the sole writer of
        # /arm_*/cartesian_reference and publishes the BLENDED reference, so
        # F_sync tethers the handle to that blended reference (not the raw user
        # pose). NOTE: this pairs the clutch/Virtual-Fixture framework with
        # blending-only assist, a conceptually non-native combination included to
        # complete the full 2x2x2 study factorial.
    ...
    def control_loop(self):
        ...
        if self.grasp_active:
            ...
            # goal-directed guidance channel is DELETED. F_guide (velocity field)
            # and F_fixture (position funnel) are NOT applied -- only F_sync is
            # rendered. Because ASSIST_BLENDING is on, main_shared_autonomy
            # publishes the BLENDED reference on /arm_*/cartesian_reference, so
            # F_sync tethers the handle to that blended reference. (f_guide/f_fix
            # are still computed above for the telemetry plots, but contribute 0
            # here; F_cbf stays disabled, exactly as in CFB.)
            f_cbf_s = np.zeros(6)
            f_guide_s = np.zeros(6)
            f_fix_s = np.zeros(6)

            # Normal running force: F_sync only (F_cbf/F_guide/F_fixture = 0).
