...
class HapticForceManagerJFB(Node):
    ...
    def __init__(self):
        ...
        self.GUIDE_K = 0.55
        # Position saturates at 1.44x, orientation at 1.2x, relative to GUIDE_K's deadzone-exit-force baseline.
    ...
    def compute_F_guide(self):
        ...
        # Feed-forward: F = MAX*tanh(D*v_field/MAX), gain applied AFTER the tanh so it reads as a
        # fraction of the exit force; no velocity feedback (passivity), vanishing as v_field -> 0 at the goal.
        F_dir = np.zeros(6)
