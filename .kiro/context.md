...
- **Clutch orientation-alignment torque (all clutch cells, `ENABLE_CLUTCH_ALIGN = True`):** while the clutch button is held the handle renders **zero linear force + a live alignment torque only**, so the operator repositions the 3D pose freely (device joints 1–2) while the wrist self-aligns to the gripper. `K_align = 5 Nm/rad` toward the **real** EE orientation (`rot_real`, recomputed every tick — not the frozen `rot_target`, which stops updating the instant clutch engages), faded to zero within `CLUTCH_ALIGN_FADE_MARGIN = 0.35 rad` of a device joint limit. Treated as a **sync effect**, so it is present in the baseline C too. Setting the flag `False` restores the legacy behaviour (whole wrench frozen at 50% for the press duration); the frozen-wrench branch is kept intact behind it.
  - **Frame handling — the reason this was previously dead code.** The error must be built by frame-mapping the **rotation vector**: conjugating a rotation by the 180°-about-Z map flips its axis x/y and leaves the angle intact, so the error stays a genuine rotation within one frame. The original implementation instead multiplied differently-framed matrices (`rot_target.as_matrix() @ rot_haption.as_matrix().T`) and tried to patch it by negating the **output torque's** x/y — which does not compose, making the torque non-restorative; it was disabled for exactly that reason. Verified numerically: the rotvec form drives the error to ~0 from arbitrary initial orientations, the matrix-mix form diverges.
  - **Two anchors, mapping a DELTA — not an absolute orientation.** Structure copied from `teleop_triago_joystick._update_home_orientation`: `Δ_triago = log(rot_real · rot_real_refᵀ)`, then `R_target_haption = exp(FRAME_FLIP ⊙ Δ_triago) · rot_haption_ref`. `(rot_real_ref, rot_haption_ref)` are captured **once on the first clutch press and never re-anchored** (the joystick's equivalents are `grip_ref_rot` + `neutral_rot`). This is mandatory, not stylistic: the gripper's and handle's zero orientations are unrelated, so only deltas are transferable between the frames — and unlike JOYSTICK, **CLUTCH teleop integrates orientation purely incrementally** (`ref_rot = delta_rot · ref_rot`) and never stores an absolute handle↔gripper relation, so the convention has to be pinned explicitly. A first implementation mapped `rot_real` absolutely (`exp(FRAME_FLIP ⊙ log(rot_real))`), which silently assumes gripper-identity == handle-identity; it felt like a smooth pull toward a plausible-but-wrong orientation. Verified numerically against a synthetic constant frame offset: the two-anchor form tracks the true matching handle orientation to 0.0000°, the absolute form is off by up to ~158°.
  - **Consequence of capture-on-first-press:** whatever orientation the handle is held in at the very first clutch press is *defined* as "matched" to the gripper's orientation at that instant. Every later press aligns against that convention, so a sloppy first press biases the whole session — re-launch the force manager to re-capture.
  - **Stiffness and saturation.** `K_align` is a live ROS parameter (`ros2 param set <node> K_align <value>`), re-read every tick, default **`0.3 Nm/rad`**. Reference points on this device: the clutch sync angular spring was live-swept to **`0.1`**, the joystick homing spring runs **`0.75`**, and **`Kp_sync_ang = 0.9` was measured UNSTABLE** — so the usable window is roughly `0.1–0.75`. The torque is soft-saturated by a tanh on its **magnitude** (`MAX_ALIGN_TORQUE = 0.35 Nm`, under the `0.5 Nm` device clip), with the axis preserved exactly so it stays restorative. Magnitude-tanh rather than per-component tanh matters here: per-component would distort the rotation axis and break restorativeness.
  - **Why not a hard clip:** in align mode `f_total` is assigned *after* the `MAX_TOTAL_*` rescale, so the only other bound is the device clip. A stiff spring that hard-clips is not a spring — it is bang-bang, max torque flipping sign as the handle crosses the target, which self-excites into a limit cycle. The first implementation shipped `K_align = 5 Nm/rad` (inherited from the dead code, ~50× the validated clutch angular stiffness) against a hard clip reached at ~5.7°, and it oscillated/diverged on contact with hardware.
...
- **Vibration cue pattern (all 8 cells, one unified design):** every cue (clutch `LIMIT_VIB_AMP` joint-limit + `GRASP_VIB_AMP` grasp; joystick `VIB_AMP` out-of-deadzone + `GRASP_VIB_AMP` grasp) is a flat `0.009 Nm` square wave, sign toggled every 150 Hz tick (~75 Hz), applied to torque axes only, firing exactly while its trigger condition holds — no timer, no arming/re-arm state. The clutch joint-limit cue used to be a one-shot 1 s burst re-armed only by a full clutch press-release cycle (`compute_F_limit_warning`, `LIMIT_VIB_DURATION`/`limit_vib_armed`/`limit_vib_active`); rewritten to the same continuous while-condition-holds pattern as the joystick cue for a genuinely unified design, buzzing whenever `min_margin <= LIMIT_OUTER`. `C.py` also had a dead `ENABLE_VIBRATION_CUES = False` flag silencing BOTH its cues entirely (unique to `C`, not present in CF/CB/CFB) — removed, cues are now unconditional there too, matching every other cell.
